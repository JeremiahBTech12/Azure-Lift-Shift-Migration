...
resource "aws_security_group" "source_vm" {
  name        = "asg-migrate-source-${var.yourname}"
  ...
  ingress {
    description = "WinRM for AzureMigrate appliance discovery"
    from_port   = 5985
    to_port     = 5985
    protocol    = "tcp"
    cidr_blocks = ["0.0.0.0/0"]
...
ingress {
  description = "SMB for ASR mobility server install"
  from_port   = 445
  to_port     = 445
  protocol    = "tcp"
  cidr_blocks = ["0.0.0.0/0"]
...
ingress {
  description = "RPC endpoint mapper for WMI"
  from_port   = 135
  to_port     = 135
  protocol    = "tcp"
  cidr_blocks = ["0.0.0.0/0"]
...
ingress {
  description = "Dynamic RPC ports for WMI"
  from_port   = 49152
  to_port     = 65535
  protocol    = "tcp"
  cidr_blocks = ["0.0.0.0/0"]
