...
az network public-ip create `
  ...
  --sku Standard
```
The NIC's IP configuration name wasn't the Azure-default `ipconfig1` — it inherited a name from the discovered server instead. Found the actual name first:
```powershell
$nicName = az network nic list --resource-group rg-migrate-target-jeremiah --query "[0].name" -o tsv
az network nic ip-config list --resource-group rg-migrate-target-jeremiah --nic-name $nicName --query "[].name" -o tsv
```
This returned `nic-EC2AMAZ-UK30H1V-00-ipConfig`, which was then used in the actual update:
```powershell
...
The replication appliance is by far the largest cost driver in this phase — with cutover now complete and verified, the discovery appliance, replication appliance, and their disks serve no further purpose and should be deallocated or deleted immediately. This project also accumulated a couple of orphaned, unattached public IPs from earlier test migrations along the way — worth a sweep, since an unattached Standard SKU public IP still bills hourly even when nothing is using it:
```powershell
az network public-ip list --query "[?ipConfiguration==null].[resourceGroup,name]" -o table
```
